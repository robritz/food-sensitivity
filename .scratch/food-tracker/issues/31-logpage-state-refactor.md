...
- [ ] Reduce/organize `LogPage`'s state, which spans (at least) three responsibilities: the add-food/category picker, the add-entry form (status, reasons, notes, intensity, occurredAt, photos, location), and the entries/queue list + detail/edit/delete dialogs.
